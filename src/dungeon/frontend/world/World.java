...
public class World extends GameWorld {
	...
	// Passes the dungeon manager to the GameWorld Parent (manager is stored for
	// reference later).
	public World(final DungeonManagerInterface mainManager) {
		super(mainManager);
	}
	...
	@Override
	public void draw() {
		final AdvancedGraphics pen = getPen();
		// drawGridLines();
		final DungeonManagerInterface mainManager = getManager();
		final int pX = mainManager.getPlayerX();
		final int pY = mainManager.getPlayerY();
		new Point(pX, pY);

		for (int x = 0; x < 16; x++) {
			for (int y = 0; y < 11; y++) {
				final int coordX = pX + x;
				final int coordY = pY + y;

				pen.setColor(new Color(255, 0, 0));
				final CellType cellType = mainManager.getCellTypeAt(coordX,
						coordY);
